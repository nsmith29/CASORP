...
from Core.CentralDefinitions import Geo_Settings, End_Error, Dirs, TESTING, ProcessCntrls
...
def CntrolGeometry():
    print("process within branch [DP.G L15]")
    t1 = th.Thread(target=GeometryProcessing, args=())
    t2 = th.Thread(target=DefProcessing, args=())
...
class GeometryProcessing(MethodFiles):
    ...
    def option2(self2, kl, extension, flpath, Q):
        ...
        if len(flpath) > 1:
            flpath.remove(Geo_Settings().perf_lxyz)
...
class DefProcessing():#MethodFiles):
    def __init__(self2):
        Files4DefiningDefect('geometry', 'defect defining', ignore=True) if 'charges and spins' in\
                                ProcessCntrls.processwants else Files4DefiningDefect('geometry', 'defect defining')
        print("done [DP.G L46]")
