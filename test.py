...
class ProcessCntrls:
    def __init__(self):
        self._processwants, self._setup, self._processresults = ['charges and spins', 'geometry'], None, {"perfect": dict(),
                                                                                              "defect": dict()}
    @property
...
def checkinput(func):
    def wrapper(Q: str, typ: str, ans: list, **kwargs) -> str:
            ...
            if Ctl_Settings.defining_except_found is True:
                ...
                async for n, r, c, e in keylist_iterator(dif_chrgs):
                    if str(self2.flexts_[1][1] + "*") not in Dirs().address_book['defect'][n][r][c].keys():
                        print('keylist', k, 'extension', et, 'dif_chrg', n, r, c, e)
                        Dirs.address_book = await add2addressbook(['defect', n, r, c], [str(paths+'*')], [flpath], Dirs().address_book)
    ...
    async def option4defect(self2, k, e):
        ...
        async for n, r, c in keylist_iterator(dif_chrgs):
            ...
            if self2.flexts_[1][1] in Dirs().address_book['defect'][n][r][c].keys():
                print('keylist', k, 'extension', e, 'dif_chrg', n, r, c,
                      Dirs().address_book['defect'][n][r][c][self2.flexts_[1][1]][0])
                ...
                if e == str(Dirs().address_book['defect'][n][r][c][self2.flexts_[1][1]][0]).split('/')[-1]:
                    ...
                    Dirs.address_book = await add2addressbook(k, [str(self2.flexts_[1][1] + '*')], [path], Dirs().address_book)
                    print('FOUND IN', n, r, c, Dirs().address_book[k[0]][k[1]][k[2]][k[3]])
                    break
...
async def ThreadOne(e1):
    ...
    if CaS_Settings().bader_missing is True:
        print('bader missing is True')
        if CaS_Settings().nn_and_def is True:
            print('nn and def wanted is True')
    ...
    print('no longer waiting for e1.2 [code L1418]')
...
class BaderProcessing(MethodFiles):
    ...
    async def option4(self2, kl, extension):
        ...
        try:
            ...
            # print(CaS_Settings.dirs_missing_bader)
...
class GeometryProcessing(MethodFiles):
    ...
    async def setoffassessment(cls):
        ...
        for type_, flexts in zip(self2.types, self2.flexts_):
            self2.type_, self2.flexts = type_, flexts
            # print(self2.type_, self2.flexts)
            await super().assessment_tree(self2, self2.type_, self2.flexts, self2.sect)
        # print('finished setoffassessment')
...
if __name__ =='__main__':
    ...
    proxylist.append(ProcessCntrls().processwants)
